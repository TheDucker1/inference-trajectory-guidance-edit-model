...
class VIBESanaEditingPipeline(SanaPipeline):
    ...
    def prepare_inputs_batch(
        ...
        return {key: value.to(self.text_encoder.device) for key, value in inputs.items()}
    ...
    def prepare_inputs_for_meta_queries(
        ...
        for batch_idx in range(batch_size):
            ...
            meta_queries = self.transformer.meta_queries.to(device=valid_embeds.device, dtype=valid_embeds.dtype)
            updated_embeds = torch.cat([valid_embeds, meta_queries, pad_embeds], dim=0)
    ...
    def get_rope_index(
        ...
        self.text_encoder.model.rope_deltas = rope_deltas.to(self.text_encoder.device)
        return position_ids
    ...
    def prepare_initial_input_embeddings(
        ...
        num_meta_queries = int(self.transformer.meta_queries.shape[0])
        img_input_ids = torch.full(
            (input_ids.shape[0], num_meta_queries),
            -1,
    ...
    def _get_editing_embeddings(
        ...
        # Align the tensor to the transformer's CURRENT device before calling a sub-module directly.
        # (It will be moved back to the execution device in the __call__ method).
        meta_hidden_states = meta_hidden_states.to(
